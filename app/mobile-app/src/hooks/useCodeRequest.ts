...
/** What a send came back with — the shape both request calls answer in. */
 ...
 *
 * Password recovery and Continue with OTP differ only in WHICH call sends and
 * what a correct code does afterwards — so each passes its `send` in and keeps
 * its own verify, reaching state through `advance` and `fail`. The mechanics
 * living once is what keeps the two flows' cooldowns, refusals and error
 * handling from drifting (rule 40). mWeb twin:
 * app/mweb/src/components/password-recovery/useCodeRequest.ts.
 */
export function useCodeRequest(
  ...
  */
  ...
  const sendCode = useCallback(
    ...
    [fallbackMessage, send, state.channel],
  );
  ...
  const fail = useCallback(
    (e: unknown) => setError(toErrorMessage(e, fallbackMessage)),
    [fallbackMessage],
  );
