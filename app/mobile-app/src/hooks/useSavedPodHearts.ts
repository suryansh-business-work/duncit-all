...
import { patchMe } from '@/stores/me.store';
 ...
 * mutation answers with the whole saved list, which is patched onto the `me`
 * store so every reader of `saved_pod_ids` agrees without re-reading the user
 * (the Saved screen queries the server directly and picks it up on focus).
 *
 * `isSaving` covers the mutation's round trip, and its answer is what flips the
 * icon — so the spinner never clears onto the old state.
 */
  ...
  const toggle = useCallback((podDocId: string) => {
    ...
    fireAndForget(
      graphqlRequest(ToggleSavedPodDocument, { podDocId }, { auth: true })
        .then((res) => patchMe({ saved_pod_ids: res.toggleSavedPod.saved_pod_ids }))
