...
import { sortShopProducts, type ShopProduct, type ShopSort } from '@/screens/ShopScreen';
...
const toOptions = (categories: Category[], level: string, parentId: string): Option[] =>
  categories
 ...
 * unavailable products are hidden), debounced search and sort. Returns the
 * filtered+sorted list plus the cascade option lists. Twin of mWeb's
 * useShopFilters. */
export function useShopFilters(categories: Category[], products: ShopProduct[]): ShopFilters {
  ...
  const visible = useMemo(() => {
    ...
    const keep = (p: ShopProduct) => {
      ...
      if (min > 0 && p.review_summary.average_rating < min) return false;
      if (!term) return true;
      return Boolean(
  ...
  }, [products, categories, effectiveCategoryId, includeOutOfStock, minRating, search, sort]);
