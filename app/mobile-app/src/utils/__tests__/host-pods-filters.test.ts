...
import { podStatus } from '@/utils/pod-format';
...
describe('podStatus', () => {
  it('derives UPCOMING / LIVE / ENDED and treats missing or bad dates as upcoming', () => {
    expect(podStatus(iso(DAY))).toBe('UPCOMING');
    expect(podStatus(iso(-HOUR), iso(HOUR))).toBe('LIVE');
    expect(podStatus(iso(-10 * HOUR))).toBe('ENDED'); // no end → 4h tail already passed
...
describe('filterHostPods', () => {
  it('returns everything when all groups are ALL', () => {
    expect(
      filterHostPods(all, withFilters({ type: 'ALL', time: 'ALL', price: 'ALL' })),
    ).toHaveLength(4);
  });

  it('filters by type (physical vs virtual)', () => {
    expect(filterHostPods(all, withFilters({ time: 'ALL', type: 'VIRTUAL' }))).toEqual([
      live,
      undated,
    ]);
    expect(filterHostPods(all, withFilters({ time: 'ALL', type: 'PHYSICAL' }))).toEqual([
      upcoming,
      ended,
    ]);
  });
  ...
  it('filters by price (paid vs free)', () => {
    expect(filterHostPods(all, withFilters({ time: 'ALL', price: 'FREE' }))).toEqual([
      live,
      undated,
    ]);
    expect(filterHostPods(all, withFilters({ time: 'ALL', price: 'PAID' }))).toEqual([
      upcoming,
      ended,
    ]);
  });
