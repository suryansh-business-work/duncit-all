import { gql, useMutation, useQuery } from '@apollo/client';
import { useCallback, useMemo } from 'react';

const SAVED_POD_IDS = gql`
  query SavedPodIds {
    me {
      user_id
      saved_pod_ids
    }
  }
`;

const TOGGLE_SAVED_POD_CARD = gql`
  mutation ToggleSavedPodCard($pod_doc_id: ID!) {
    toggleSavedPod(pod_doc_id: $pod_doc_id) {
      pod_id
      saved
      saved_pod_ids
    }
  }
`;

/**
 * The heart on pod cards: whether a pod (by DOC id) is in the viewer's saved
 * list, plus the toggle. The Saved page reads the same `saved_pod_ids`, so a
 * toggle here updates it too.
 */
export function useSavedPodHearts() {
  const { data } = useQuery<{ me?: { user_id: string; saved_pod_ids: string[] } | null }>(
    SAVED_POD_IDS,
    { fetchPolicy: 'cache-and-network' }
  );
  const [toggleMut] = useMutation(TOGGLE_SAVED_POD_CARD, {
    refetchQueries: [{ query: SAVED_POD_IDS }],
  });

  const ids = useMemo(() => new Set(data?.me?.saved_pod_ids ?? []), [data?.me?.saved_pod_ids]);
  const isSaved = useCallback((podDocId: string) => ids.has(podDocId), [ids]);
  const toggle = useCallback(
    (podDocId: string) => {
      toggleMut({ variables: { pod_doc_id: podDocId } }).catch(() => undefined);
    },
    [toggleMut]
  );

  // Signed-out visitors have no saved list — the cards hide their heart then.
  return { isSaved, toggle, signedIn: !!data?.me?.user_id };
}
