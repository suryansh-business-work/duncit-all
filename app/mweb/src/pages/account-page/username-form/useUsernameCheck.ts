  ...
  useEffect(() => {
    ...
    const timer = globalThis.setTimeout(() => {
      client
        ...
        .catch((error) => {
          if (latest.current !== candidate) return;
          // An unreachable check is not a refusal: leave the field in its
          // checking state rather than telling somebody their handle is taken
          // because the network blinked.
          logs.mWeb.error('useUsernameCheck', 'availability', { error, candidate });
          setCheck({ checking: false, available: null, reason: null });
        });
    }, DEBOUNCE_MS);
