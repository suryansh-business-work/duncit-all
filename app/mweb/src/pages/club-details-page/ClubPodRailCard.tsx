...
/** The locality chip's row (24px chip + its 6px gap), taken off the photo so a
 * card with the chip is as tall as one without it. */
const LOCALITY_ROW = 30;
 ...
 * that opens the pod, then the pod's locality chip.
 */
export default function ClubPodRailCard({ pod, priceFormat, onOpen }: Readonly<Props>) {
  ...
  const hasLocality = Boolean(pod.locality?.trim());
  // 18px media corners inside the card's padding, like the Home pod card.
  const mediaSx = { height: MEDIA_HEIGHT - (hasLocality ? LOCALITY_ROW : 0), borderRadius: '18px' };

  return (
    <Card data-testid={`pod-card-${pod.id}`} sx={{ width: 180, flex: '0 0 auto', p: 1 }}>
      <Box sx={{ position: 'relative' }}>
        {cover ? (
          <CardMedia component="img" image={cover} alt={pod.pod_title} sx={{ ...mediaSx, objectFit: 'cover' }} />
        ) : (
          <Box sx={{ ...mediaSx, display: 'grid', placeItems: 'center', bgcolor: 'action.hover', color: 'secondary.main' }}>
            <EventIcon />
          </Box>
        )}
        <Typography
