...
// framework-free so the mWeb (@mui/x-charts) and mobile (gifted-charts) charts
...
export function hostRangeMeta(range: HostChartRange): RangeMeta {
  ...
  if (range === 'PAST_3_MONTHS') {
    return { title: 'Pods Hosted in Past 3 Months', description: 'Your hosted Pods over the last 3 months.' };
  }
  ...
  return { title: 'Pods Hosted in Past 6 Months', description: 'Your hosted Pods over the last 6 months.' };
}
...
function rangeWindow(range: HostChartRange, valid: readonly Date[]): { start: Date; end: Date } | null {
  const now = new Date();
  if (range === 'PAST_3_MONTHS') return { start: new Date(now.getFullYear(), now.getMonth() - 2, 1), end: now };
  if (range === 'PAST_6_MONTHS') return { start: new Date(now.getFullYear(), now.getMonth() - 5, 1), end: now };
  if (range === 'CURRENT_YEAR') {
...
export function buildParticipantTrend(pods: readonly ParticipantPod[]): ChartDatum[] {
  ...
  list.sort(
    (a, b) => new Date(a.pod_date_time as string).getTime() - new Date(b.pod_date_time as string).getTime(),
  );
  return list.map((p) => ({
    label: new Date(p.pod_date_time as string).toLocaleString('en', { month: 'short', day: 'numeric' }),
