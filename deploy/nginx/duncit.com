...
# --- Communications console: communications.duncit.com (SPA on :2035) ---
server {
    listen 80;
    listen [::]:80;
    http2 on;
    server_name communications.duncit.com;
    client_max_body_size 25m;
...
# --- Logs console: logs.duncit.com (SPA on :2036) ---
server {
    listen 80;
    listen [::]:80;
    http2 on;
    server_name logs.duncit.com;
    client_max_body_size 25m;

    location / {
        proxy_pass         http://127.0.0.1:2036;
        proxy_http_version 1.1;
        proxy_set_header   Host              $host;
        proxy_set_header   X-Real-IP         $remote_addr;
        proxy_set_header   X-Forwarded-For   $proxy_add_x_forwarded_for;
...
# --- Analytics console: analytics.duncit.com (SPA on :2037) ---
server {
    listen 80;
    listen [::]:80;
    http2 on;
    server_name analytics.duncit.com;
    client_max_body_size 25m;
...
# head before the HTML leaves — what a shared link's preview reads.
server {
    listen 80;
    listen [::]:80;
    http2 on;
