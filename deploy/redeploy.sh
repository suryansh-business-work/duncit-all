...
SERVICES=(server admin mweb website)
PORTS=(2001 2002 2003 2000)
...
for i in {1..30}; do
  if docker ps --format '{{.Names}} {{.Status}}' | grep -E 'duncit-(server|admin|mweb|website)' | grep -vq 'Restarting\|Created'; then
    break
  fi
  sleep 2
done
