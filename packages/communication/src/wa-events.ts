/**
 * Every WhatsApp message Duncit sends on its own — the scenario, the AiSensy
 * campaign behind it, and what fills each {{n}}.
 *
 * WHY THIS IS DATA AND NOT COPY: the admin console renders these rows, and a
 * translation key per scenario would have to be composed
 * (`t('admin.wa.' + key)`), which `scripts/verify-translation-keys.mjs`
 * cannot see — it greps for a literal `t('key')`. The screen's own chrome is
 * localized; the catalogue itself travels as data, exactly as `WA_VARIABLES`
 * already does for the marketing sender.
 *
 * WHY `params` IS ONLY LABELS: the values are read out of Mongo and can only be
 * resolved server-side. What this file pins is the ORDER and the COUNT, which is
 * the half that silently rots — AiSensy renders a positional array, so one
 * inserted placeholder shifts every later value into the wrong sentence.
 * Each label is transcribed from the approved body, so `{{2}}` of
 * `user_pod_reminder` reads "Hours until pod" and not "Pod title": that
 * template says "your Pod starts in {{2}} hours".
 *
...
export const WA_EVENTS: readonly WaEvent[] = [
  ...
  {
    key: 'HOST_POD_CANCELLATION_RISK',
    campaign: 'host_pod_cancellation_risk',
    audience: 'HOST',
    category: 'billing',
  ...
  {
    key: 'ECOMM_BRAND_SUBMITTED',
    campaign: 'ecomm_brand_submitted',
    audience: 'ECOMM',
    category: 'notification',
    fires: 'A brand is submitted for review',
    params: ['Recipient name', 'Brand'],
  },
  {
    key: 'ECOMM_BRAND_REJECTED',
    campaign: 'ecomm_brand_rejected',
    audience: 'ECOMM',
    category: 'account',
    fires: 'A submitted brand is rejected by the Products team',
    params: ['Recipient name', 'Brand', 'Reason'],
  },
  {
    key: 'ECOMM_BRAND_DELETED',
    campaign: 'ecomm_brand_deleted',
    audience: 'ECOMM',
    category: 'account',
    fires: 'A brand is deleted',
    params: ['Recipient name', 'Brand'],
  },
  {
  ...
  },
  {
    ...
    audience: 'USER',
    category: 'transactional',
    fires: 'A pet-store order is handed to the courier',
    params: ['Recipient name', 'Order number', 'Courier', 'AWB', 'Tracking link'],
  },
  {
    key: 'STORE_ORDER_OUT_FOR_DELIVERY',
    campaign: 'store_order_out_for_delivery',
    audience: 'USER',
    category: 'transactional',
    fires: 'A pet-store order is out for delivery',
    params: ['Recipient name', 'Order number', 'Tracking link'],
  },
  {
    key: 'STORE_ORDER_DELIVERED',
    campaign: 'store_order_delivered',
    audience: 'USER',
    category: 'transactional',
    fires: 'A pet-store order is delivered',
    params: ['Recipient name', 'Order number', 'Order link'],
  },
  {
  ...
  {
    key: 'CLUB_ADMIN_POD_CANCELLATION_RISK',
    campaign: 'club_admin_pod_cancellation_risk',
    audience: 'CLUB_ADMIN',
    category: 'billing',
