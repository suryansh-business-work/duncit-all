 ...
 *
 * As with the venue mapper, a `??` here is reserved for a field the query can
 * actually answer null for — `dob`, `bank_account.payout_method`,
 * `host_commission_pct`, and the three category ids. Everything else the query
 * selects is non-null, so a fallback on it would be a branch nothing reaches.
 */
 ...
 * somebody started and abandoned — or over one the server itself stored that
 * way, which is why the form schema allows them through.
 */
