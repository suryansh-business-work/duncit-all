...
import DirectMarkCta from './DirectMarkCta';
...
export default function PodAttendanceView({
  ...
  // On EVERY pod a Club Admin may still write to, empty roster included — the
  // whole point is that they no longer have to find a row to start from.
  const showDirect = canDirectMark(board);
  ...
  return (
    <Stack spacing={2} data-testid="pod-attendance-view">
      ...
      {showDirect && <DirectMarkCta label={labels.directCta} onOpen={api.openDirect} />}
      {deadlineAt && (
        <DeadlineNotice
          labels={labels}
