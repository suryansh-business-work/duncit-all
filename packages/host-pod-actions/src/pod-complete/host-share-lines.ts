 ...
 */
export function buildHostShareLines(settlement: PodSettlement): WaterfallLine[] {
  const w = settlement.waterfall;
  const venueLines: WaterfallLine[] = settlement.has_venue
    ? [
        { key: 'venue', label: 'Venue slot price', value: w.venue_amount },
        { key: 'venue-receives', label: 'Venue receives', value: w.venue_receives },
      ]
    : [];
  return [
    { key: 'paid', label: 'Customer Paid', value: w.amount },
    { key: 'gst', label: `− GST (${w.gst_pct}%)`, value: w.gst_amount },
    { key: 'fee', label: `− Platform Fee (${w.platform_fee_pct}%)`, value: w.platform_fee_amount },
    { key: 'pool', label: 'Pool', value: w.pool_amount },
    ...venueLines,
    { key: 'host', label: 'You receive', value: w.host_receives, strong: true },
    { key: 'duncit', label: 'Duncit revenue', value: w.duncit_revenue },
  ];
}
