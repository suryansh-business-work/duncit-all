...
changeRequests: {
    ...
    branding: {
      ...
      primaryColorHint:
        'Button and call-to-action colour of mWeb and the app. Fills the Primary row of both Theme tokens tables — applied only while Theme tokens is set to Server.',
      themeTokens: 'Theme tokens (mWeb + app)',
      themeTokensHint: 'Light and dark colour tokens for mWeb and the mobile app — keep the bundled ones or take them from here.',
      themeTokensInfo:
        'Leave a value blank to keep the bundled (local) one. Changes reach mWeb and the app the next time they load. Contrast shows whether a colour still meets WCAG AA against the colour it sits on or under.',
      tokenSourceLabel: 'Take theme tokens from this page (Server)',
      tokenSourceLocal: 'Off — mWeb and the app use their bundled (local) tokens. The values below are saved but not applied.',
      tokenSourceServer: 'On — mWeb and the app lay the values below over their bundled tokens.',
      tokenTableLight: 'Light mode',
      tokenTableDark: 'Dark mode',
      colToken: 'Token',
      colUsage: 'What it colours',
      colLocal: 'Local value',
      colServer: 'Server value',
      colContrast: 'Contrast',
      serverValueFor: '{token} — server value',
      invalidColor: 'Use #hex, rgb() or rgba()',
      contrastPass: '{ratio}:1 against {pair}',
      contrastFail: '{ratio}:1 against {pair} — needs {min}:1',
      contrastNone: 'Decorative — no minimum',
      tokenHints: {
        bg: 'Page background behind every screen.',
        surface: 'Cards, sheets, dialogs and input fields.',
        soft: 'Subtle fills — chips, pressed rows, skeletons.',
        ink: 'Main text and icons.',
        muted: 'Secondary text — hints, captions, timestamps.',
        border: 'Hairline dividers and card edges (decorative).',
        inputBorder: 'Form field outline — needs 3:1 against the card.',
        primary: 'Call-to-action fill — buttons, the + button, the active pill. Set from Identity → Primary color.',
        primaryHover: 'Primary button while hovered — keep it darker than Primary.',
        primaryActive: 'Primary button while pressed — darker than the hover step.',
        onPrimary: 'Text and icons on a Primary fill.',
        accent: 'Coloured text — links, “See all”, the active tab, the focus ring.',
        onAccent: 'Text on an Accent-filled chip.',
        brand: 'The exact brand colour for the logo and illustrations — never text.',
        success: 'Success text, icons and fills.',
        warning: 'Warning text, icons and fills.',
        error: 'Error text, the required asterisk and destructive actions.',
        info: 'Informational text, icons and fills.',
        onSemantic: 'Text on a filled success, warning, error or info colour.',
      },
    },
