...
export const SHELL_BUNDLE: NestedCatalogue = {
  ...
  shell: {
    ...
    jobs: {
      ...
      // AI translation runs (Localization console): a key per row, not a row.
      translating: {
        one: 'Translating {count} key',
        other: 'Translating {count} keys',
      },
      translated: {
        one: 'Translated {count} key',
        other: 'Translated {count} keys',
      },
      ...
      translatePartialNotice: '{label}: {translated} translated, {failed} came back unusable — the next sync retries them.',
      translateStoppedNotice: '{label}: the translation stopped — {reason}',
    },
  ...
  tech: {
    ...
    appBuilds: {
      ...
      colAppStore: 'App Store',
      appStoreTestflight: 'TestFlight',
      appStoreReview: 'App Store',
      appStoreTrackTestflight: 'TestFlight',
      appStoreTrackReview: 'App Review',
      appStorePush: 'Push to {track}',
      appStorePushingTip: '{track}: {stage} — since {when}…',
      appStoreReleasedTestflightTip: 'Processed and on TestFlight since {when} — pushed by {by}.',
      appStoreReleasedReviewTip: 'Submitted to App Review on {when} — pushed by {by}. Apple reports the outcome in App Store Connect.',
      appStoreFailedTip: 'Push to {track} failed: {error} Press to try again.',
      appStoreUnavailable:
        'Only a successful production build with a stored IPA and a build number can go to App Store Connect.',
      appStoreConfirmTitle: 'Push {build} to {track}?',
      appStoreConfirmTestflight:
        'The IPA for v{version} is uploaded to App Store Connect from this server — no Mac involved — and processed for TestFlight. Testers with automatic access get it once Apple finishes, usually within half an hour.',
      appStoreConfirmReview:
        'The IPA for v{version} is uploaded, Store Listing is applied to the App Store version, and the version is submitted to App Review. Once Apple approves it, it goes live on its own. This cannot be taken back from here.',
      appStoreConfirmAction: 'Push',
      appStoreStarted: '{build} is on its way to {track}. The row shows each step; Apple’s processing takes a while.',
      appStoreReleasesTitle: 'App Store pushes',
      appStoreNoReleases: 'This build has not been pushed to TestFlight or the App Store.',
      appStoreStatusPushing: 'Pushing',
      appStoreStatusFailed: 'Failed',
      appStoreDoneTestflight: 'On TestFlight',
      appStoreDoneReview: 'Submitted for review',
      appStoreReleaseBy: 'by {by} · {when}',
      appStoreBuildId: 'Apple build {id}',
      // --- Releases: what the stores say about every version ---------------
      releasesTitle: 'Releases',
      releasesSubtitle:
        'Every version as the App Store and Google Play show it right now — review state, rejections with their reason and what to do, and the latest build one click from a retry.',
      releasesTabsLabel: 'Store',
      releaseStoreAppStore: 'App Store',
      releaseStoreGooglePlay: 'Google Play',
      releasesReadAt: 'Read from {app} at {when}',
      releasesNotConfiguredShort: 'Not connected yet',
      releasesRefresh: 'Refresh',
      releasesOpenSettings: 'Open Settings',
      releasesReadFailed: 'The store could not be read: {error}',
      releasesPlayNoReviewApi:
        'Google Play’s API reports tracks and rollouts only — never a review outcome. When Play rejects a release, press Log rejection and paste the reason from the Play Console mail; the advice, the mail and the Slack post follow from that.',
      releasesEmpty: 'The store lists no releases yet.',
      releasesSearchPlaceholder: 'Search version, build, state or reason',
      releaseOpenStore: 'Open in store console',
      releaseColCreated: 'Created',
      releaseColBuild: 'Build number',
      releaseColStoreState: 'Store status',
      releaseColReview: 'Review',
      releaseColSubmitted: 'Submitted',
      releaseColTrack: 'Track · rollout',
      releaseColIssue: 'Rejection / action',
      releaseColResubmitted: 'Resubmitted',
      releaseColBuildRow: 'Build row',
      releaseStatusPreparing: 'Preparing',
      releaseStatusWaiting: 'Waiting for review',
      releaseStatusInReview: 'In review',
      releaseStatusApproved: 'Approved',
      releaseStatusLive: 'Live',
      releaseStatusTesting: 'Testing',
      releaseStatusRollingOut: 'Rolling out',
      releaseStatusHalted: 'Halted',
      releaseStatusRejected: 'Rejected',
      releaseStatusWithdrawn: 'Withdrawn',
      releaseStatusReplaced: 'Replaced',
      releaseStatusRemoved: 'Removed',
      releaseStatusOther: 'Other',
      releaseIssueRejection: 'Rejected',
      releaseIssueAwaiting: 'Awaiting release',
      releaseIssueResolvedShort: '{kind} · resolved',
      releaseIssueOpen: 'Still open',
      releaseDialogTitle: 'Version {version} · build {build}',
      releaseNoIssue:
        'The store reports nothing wrong with this release. If it was rejected and the store’s API does not say so, log the rejection with the reviewer’s reason.',
      releaseMeaningRejected:
        'App Review refused the app itself — behaviour, a crash, a missing feature or a guideline the build does not meet. The reviewer’s message is in App Store Connect › Resolution Center; paste it below.',
      releaseMeaningMetadataRejected:
        'App Review refused the listing — text, screenshots, review notes or the demo account — not the binary. Fix Store Listing and submit the same build again.',
      releaseMeaningInvalidBinary:
      ...
      releaseMeaningDeveloperRejected: 'Someone on our side withdrew this version from review.',
      releaseMeaningPendingDeveloperRelease:
        'Apple approved this version. It goes live only when someone presses Release this version in App Store Connect.',
      releaseMeaningManual: 'Logged by hand from the store’s mail or console.',
      releaseMeaningOther: 'The store reports this version as {state}.',
      releaseFactIssue: 'Issue',
      releaseFactFirstSeen: 'First seen',
      releaseFactResolved: 'Resolved',
      releaseFactNotices: 'Notices',
      releaseFactResubmitted: 'Resubmitted',
      releaseSourceStore: 'reported by the store',
      releaseSourceManual: 'logged by {by}',
      releaseResolvedStateChanged: 'Store moved to {state} · {when}',
      releaseResolvedBy: 'Closed by {by} · {when}',
      releaseResolvedGone: 'The store no longer lists it · {when}',
      releaseNoticePending: 'Not sent yet',
      releaseNoticeSent: 'Mail and Slack sent {when}',
      releaseNoticeFailed: 'Sending failed: {error}',
      releaseReminders: '{times} reminder(s), last {when}',
      releaseResubmittedLabel: '{build} by {by} · {when}',
      releaseAdviceTitle: 'What to do',
      releaseAdvicePending: 'The advice is being written — it takes a few seconds after a rejection is first seen. Refresh to read it.',
      releaseAdviceNone: 'No advice could be written for this issue.',
      releaseAdviceCauses: 'Likely causes',
      releaseAdviceSteps: 'Steps now',
      releaseAdviceNextTime: 'Next time',
      releaseAdviceMeta: 'Written by {model} · {when}',
      releaseConfidenceHigh: 'High confidence',
      releaseConfidenceMedium: 'Medium confidence',
      releaseConfidenceLow: 'Low confidence — add the reviewer’s message',
      reviewerMessageTitle: 'Reviewer’s message',
      reviewerMessageLabel: 'What the reviewer said',
      reviewerMessageHint:
        'Paste it from App Store Connect › Resolution Center or the Play Console mail. Neither store’s API carries it, and the advice is only as good as this.',
      reviewerMessageSave: 'Save and write the advice again',
      reviewerMessageSaved: 'Reviewer’s message saved and the advice rewritten.',
      reviewerMessageTooShort: 'Paste at least {min} characters of the reviewer’s message.',
      reviewerMessageTooLong: 'Keep the message under {max} characters.',
      logRejectionAction: 'Log rejection',
      logRejectionTitle: 'Log a rejection',
      logRejectionSubtitle:
        'For a rejection the store’s API cannot report — every Google Play one. The advice, the mail and the Slack post follow at once.',
      logRejectionVersionHint: 'As the store shows it, e.g. 1.80.3.',
      logRejectionBuildHint: 'CFBundleVersion or version code, if you have it.',
      logRejectionVersionRequired: 'Name the version that was rejected.',
      logRejectionVersionTooLong: 'Keep the version under 120 characters.',
      logRejectionBuildTooLong: 'Keep the build number under 60 characters.',
      logRejectionSubmit: 'Log and advise',
      logRejectionDone: 'Rejection logged — the advice is written and the notices are on their way.',
      submitLatestAction: 'Submit latest build',
      submitLatestTitle: 'Submit the latest build to {store}?',
      submitLatestMessage:
        'The newest successful production build with a stored artifact is pushed to {store} for review, with the Store Listing applied. Once approved it reaches every user.',
      submitLatestStarted: 'Push started for {build} (v{version}). Follow it on the Android or iOS table.',
      resolveIssueAction: 'Mark resolved',
      resolveIssueTitle: 'Mark this issue resolved?',
      resolveIssueMessage: 'The reminders for version {version} stop. The store’s own state is not changed.',
      resolveIssueDone: 'Issue marked resolved.',
      releaseNoticesTitle: 'Release notices',
      releaseNoticesHint:
        'Where a rejection — or an approved version waiting to be released — is announced, and how often it is raised again while it stays open. Every notice carries the AI advice.',
      releaseNoticesNoSlack: 'Slack is not connected, so only the mail goes out. Connect a bot on Environment Variables › Slack to post as well.',
      releaseNoticesEnabled: 'Send notices',
      releaseNoticesEnabledHint: 'Off means nothing is mailed or posted; the issues are still recorded and shown on Releases.',
      releaseNoticesChannel: 'Slack channel',
      releaseNoticesChannelHint: 'Pick a channel the bot has joined, or paste a channel ID. Leave empty for mail only.',
      releaseNoticesMailTo: 'Mail to',
      releaseNoticesMailToHint: 'One address per line, or comma-separated. At least one.',
      releaseNoticesMailRequired: 'Add at least one address.',
      releaseNoticesMailInvalid: 'One of the addresses is not a valid email address.',
      releaseNoticesReminders: 'Send reminders',
      releaseNoticesRemindersHint: 'While an issue stays open, the notices go out again every reminder window — until it is resolved or the store moves on.',
      releaseNoticesHours: 'Reminder window (hours)',
      releaseNoticesHoursRange: 'A whole number from {min} to {max} hours.',
    },
    storeListing: {
      title: 'Store Listing',
      subtitle:
        'What App Store Connect and Google Play ask for when the app is listed. Filled once here; every Push to App Store and every Push to Google Play production applies it.',
      save: 'Save listing',
      saving: 'Saving…',
      saved: 'Store listing saved.',
      lastSaved: 'Last saved by {by} on {when}.',
      tabCopy: 'Copy & links',
      tabScreenshots: 'Screenshots',
      tabReview: 'Review & contact',
      tabChecklist: 'One-time setup',
      sectionShared: 'Both stores',
      sectionApple: 'App Store only',
      sectionPlay: 'Google Play only',
      locale: 'Locale',
      localeHint: 'BCP-47 tag both stores accept, e.g. en-US. The listing is written in this one language.',
      name: 'App name',
      nameHint: 'App Store name and Google Play title. Up to {max} characters.',
      subtitleField: 'Subtitle',
      subtitleHint: 'Shown under the name on the App Store. Up to {max} characters.',
      shortDescription: 'Short description',
      shortDescriptionHint: 'Google Play’s one-line summary. Up to {max} characters.',
      description: 'Description',
      descriptionHint: 'The full listing text on both stores. Up to {max} characters.',
      keywords: 'Keywords',
      keywordsHint: 'App Store search terms, comma-separated. Up to {max} characters.',
      whatsNew: 'What’s new',
      whatsNewHint: 'Release notes for this version, on both stores. Up to {max} characters.',
      copyright: 'Copyright',
      copyrightHint: 'The year and the owner, e.g. 2026 Duncit — no URL or web address. Apple requires it to submit.',
      copyrightNoUrl: 'Apple refuses a URL or web address here. Write the year and the owner, e.g. 2026 Duncit.',
      primaryCategory: 'Primary category',
      primaryCategoryHint:
        'Apple’s list, read live from App Store Connect. Google Play’s category is set once in the Play Console.',
      primaryCategoryUnavailable: 'Connect App Store Connect under Environment Variables to load Apple’s categories.',
      privacyPolicyUrl: 'Privacy policy URL',
      privacyPolicyUrlHint: 'Shown on the App Store listing. On Google Play it is set once under App content.',
      supportUrl: 'Support URL',
      supportUrlHint: 'App Store support link and Google Play contact website.',
      marketingUrl: 'Marketing URL',
      marketingUrlHint: 'Optional. The app’s website, shown on the App Store.',
      contactEmail: 'Contact email',
      contactEmailHint: 'Public on Google Play; App Review’s contact on Apple.',
      contactPhone: 'Contact phone',
      contactPhoneHint: 'With country code, e.g. +91… Apple requires it for App Review.',
      reviewFirstName: 'Review contact first name',
      reviewLastName: 'Review contact last name',
      demoAccountRequired: 'App Review needs an account to sign in',
      demoAccountName: 'Demo account',
      demoAccountNameHint:
        'The sign-in App Review uses. It must work with a password or a fixed code — a reviewer cannot receive a live WhatsApp OTP.',
      demoAccountPassword: 'Demo account password',
      reviewNotes: 'Notes for App Review',
      reviewNotesHint:
        'Where to find things, how sign-in works, anything the reviewer should know. Up to {max} characters.',
      iphoneScreenshots: 'iPhone screenshots',
      iphoneScreenshotsHint: '1–10 PNG or JPEG at 1290×2796 or 1320×2868 (6.7″ / 6.9″). Required.',
      ipadScreenshots: 'iPad screenshots',
      ipadScreenshotsHint: '1–10 at 2064×2752 or 2048×2732 (13″). Required — the app supports iPad.',
      androidPhoneScreenshots: 'Android phone screenshots',
      androidPhoneScreenshotsHint: '2–8 PNG or JPEG, 16:9 or 9:16, 320–3840 px on each side. Required.',
      androidTablet7Screenshots: '7″ tablet screenshots',
      androidTablet10Screenshots: '10″ tablet screenshots',
      androidTabletHint: 'Optional, up to 8.',
      androidFeatureGraphic: 'Feature graphic',
      androidFeatureGraphicHint: '1024×500 PNG or JPEG, no transparency. Required by Google Play.',
      androidIcon: 'App icon',
      androidIconHint: '512×512 PNG, 32-bit. Required by Google Play.',
      upload: 'Upload',
      required: 'Required.',
      tooLong: 'At most {max} characters.',
      invalidUrl: 'Must be an https:// URL.',
      invalidEmail: 'Must be an email address.',
      tooFewImages: 'At least {min} images.',
      tooManyImages: 'At most {max} images.',
      checklistTitle: 'Done once, by hand, in each console',
      checklistIntro:
        'Neither store lets these be set through its API. Each is asked once and kept across versions; a push fails with the store’s own message while one is missing.',
      checklistApple: 'App Store Connect',
      appleCreateApp:
        'Create the app record under Apps → New App, with the bundle ID from Environment Variables → App Store Connect. A push refuses until it exists.',
      appleAgreements: 'Accept the Free Apps agreement under Business → Agreements, or submission stops at PENDING_CONTRACT.',
      appleAgeRating: 'Answer the age-rating questionnaire once: App Information → Age Rating.',
      applePricing: 'Set price and availability once: Pricing and Availability → Free, all territories.',
      appleContentRights: 'Answer the content-rights question once: App Information → Content Rights.',
      appleTrader: 'Fill in trader status for the EU: Business → Digital Services Act.',
      applePrivacyLabels: 'Fill in App Privacy (the data-collection labels) once under App Privacy.',
      checklistPlay: 'Google Play Console',
      playFirstRelease: 'Make the first release by hand once — the API cannot create the app or its listing.',
      playPrivacyPolicy: 'App content → Privacy policy URL.',
      playDataSafety: 'App content → Data safety questionnaire.',
      playContentRating: 'App content → Content rating questionnaire.',
      playCategory: 'Store settings → App category and tags.',
      playServiceAccount:
        'Invite the service account from Environment Variables → Google Play on the app, with release rights.',
    },
    mailAutomation: {
