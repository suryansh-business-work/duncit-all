...
export const SHELL_BUNDLE: NestedCatalogue = {
  ...
  shell: {
    ...
    table: {
      ...
      // The download menu and the GET API dialog beside refresh.
      download: 'Download',
      downloadPageCsv: 'Current page as CSV',
      downloadPageJson: 'Current page as JSON',
      downloadAllCsv: 'All rows as CSV',
      downloadAllJson: 'All rows as JSON',
      downloadFailed: 'The download could not be prepared. Please try again.',
      getApi: 'GET API',
      apiIntro: 'This URL returns this table’s rows as JSON, with the search, sort, filters and page you are looking at now. It runs as you, so it only returns what you can already see.',
      apiNoToken: 'You do not have a table API token yet. Generate one in Tech portal → Table API → Settings, then reopen this dialog.',
      apiTokenWarning: 'This URL carries your personal token. Treat it like a password.',
      apiLoadFailed: 'Your table API access could not be loaded. Please try again.',
      apiTokenPlaceholder: 'YOUR_TOKEN',
      apiPagination: 'Pagination: set page (from 1) and page_size (1–100). Every response includes total, total_pages, next_page_url and previous_page_url.',
      apiHeaderHint: 'Scripts can send the token in the x-table-api-token header instead of the URL.',
      apiCopy: 'Copy GET URL',
      apiCopied: 'GET URL copied',
      apiCopyFailed: 'Could not copy the URL',
      refresh: 'Refresh',
    ...
    nav: {
      ...
      aiMonitoringLogs: 'AI Monitoring Logs',
      aiUsage: 'AI Usage',
      allPods: 'All Pods',
      allUsers: 'All Users',
      amenitiesManagement: 'Amenities management',
      analyticsAlerts: 'Alerts',
      ...
      badges: 'Badges',
      beAHost: 'Be a Host',
      becomeAnECommerceBrandPartner: 'Become an E-Commerce Brand Partner',
      blog: 'Blog',
      boards: 'Boards',
      branding: 'Branding',
      ...
      eCommerceBrandDashboard: 'E-Commerce Brand Dashboard',
      ...
      funnelRetention: 'Funnel & Retention',
      giftCardLogs: 'Gift Card Logs',
    ...
    coupons: {
      ...
      // The detail page one code opens onto: its rules, and what it has done.
      detailLoadFailed: 'Could not load this coupon',
      notFound: 'This coupon no longer exists.',
      rulesTitle: 'Rules',
      ...
      ofMax: 'of {max}',
      usesLeft: '{left} left',
      unlimitedUses: 'No usage cap',
      noLimit: 'No limit',
      noMinimum: 'No minimum',
      lastRedeemed: 'Last redeemed',
      neverRedeemed: 'Never',
      redemptionsTitle: 'Who used this coupon',
      redemptionsSubtitle:
        'One row per payment that spent the code. A refunded order stays listed — the use was still counted against the cap.',
      redemptionsEmpty: 'Nobody has used this coupon yet.',
      redemptionsSearch: 'Search member, email, payment or invoice',
      colMember: 'Member',
      colPayment: 'Payment',
      colFor: 'For',
      colOrderTotal: 'Order total',
      colRedeemedOn: 'Redeemed on',
      statusSuccess: 'Paid',
    ...
    richText: {
      ...
      undo: 'Undo',
      redo: 'Redo',
      // Tables. One toolbar button opens a menu, because seven more icons in a
      // row that already wraps on a phone is not a toolbar any more.
      table: 'Table',
  ...
  admin: {
    ...
    autoPods: {
      ...
      // OPEN is "nobody yet"; the venue goes first, then the host, then the club admin.
  ...
  tech: {
    ...
    appBuilds: {
      ...
      colPlayStore: 'Google Play',
      playInternal: 'Internal',
      playProduction: 'Production',
      playTrackInternal: 'internal testing',
      playTrackProduction: 'production',
      playPush: 'Push to Google Play {track}',
      playPushingTip: 'Pushing to Google Play {track} since {when}…',
      playReleasedTip: 'On Google Play {track} as version code {code} — pushed by {by} on {when}.',
      playFailedTip: 'Push to {track} failed: {error} Press to try again.',
      playUnavailable: 'Only a successful production build with a stored AAB can go to Google Play.',
      playConfirmTitle: 'Push {build} to Google Play {track}?',
      playConfirmInternal:
        'Version {version} goes to the internal testing track. Only the testers listed on it can install it, and nothing changes for users.',
      playConfirmProduction:
        'Version {version} is rolled out to EVERY user on Google Play. Google may review it first, and once it is live the only way back is a newer release.',
      playConfirmAction: 'Push',
      playStarted: 'Push to Google Play {track} started for {build}. The row updates as Google answers.',
      playReleasesTitle: 'Google Play',
      playNoReleases: 'Not pushed to Google Play yet.',
      playStatusPushing: 'Pushing…',
      playStatusReleased: 'Released',
      playStatusFailed: 'Failed',
      playReleaseBy: 'by {by} · {when}',
      playVersionCode: 'version code {code}',
      playSettingsTitle: 'Google Play',
      playSettingsConfigured: 'Connected. A push from the Android table releases to {package}.',
      playSettingsNotConfigured:
        'Google Play is not connected. Add a service account key and package name in Environment Variables → Google Play — until then, pushing a build says so.',
      playSettingsHint:
