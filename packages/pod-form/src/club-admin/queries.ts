...
/**
 * The Club Admin's pod documents — ONE copy for the Partners console and mWeb
 * (rule 40). Every read is club-scoped by the server from the caller's own
 * membership, so no consumer can widen it with a client filter.
 */
...
/** The wide selection every row carries — a table row prefills the edit form. */
export const CLUB_ADMIN_POD_ROW_FIELDS = gql`
...
/** Single-pod fetch for the edit route. Its own query rather than the public
 * pod query: a cancelled pod stays editable here, and only this one returns
 * soft-deleted rows to a club admin. */
