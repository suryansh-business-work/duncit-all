...
describe('slotIssueMessage', () => {
  it('turns every code into its own availability.* key, with the cap interpolated', () => {
    ...
    for (const code of codes) expect(slotIssueMessage(code, t)).toBe(`availability.${code}`);
    expect(slotIssueMessage('maxAhead', t)).toBe(`availability.maxAhead:${MAX_FUTURE_DAYS}`);
