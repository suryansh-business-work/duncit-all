 ...
 *  because the localization gate reads them statically (rule 38), and under the
 *  `availability.*` namespace every surface ships — mWeb and the native app do
 *  not carry the portal shell's bundle. */
export function slotIssueMessage(code: SlotIssueCode, t: Translate): string {
  if (code === 'pickDates') return t('availability.pickDates');
  if (code === 'pickSlotTimes') return t('availability.pickSlotTimes');
  if (code === 'endDateAfterStart') return t('availability.endDateAfterStart');
  if (code === 'sameTime') return t('availability.sameTime');
  if (code === 'endAfterStart') return t('availability.endAfterStart');
  if (code === 'startInFuture') return t('availability.startInFuture');
  return t('availability.maxAhead', { vars: { days: MAX_FUTURE_DAYS } });
}
