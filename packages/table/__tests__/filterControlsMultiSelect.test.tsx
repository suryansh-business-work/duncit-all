...
// comma-joined string and cover EnumControl's `value.split(',')` path.
vi.mock('@mui/material/Select', () => ({
  __esModule: true,
  default: ({ onChange }: { onChange: (e: { target: { value: unknown } }) => void }) => (
    <button type="button" data-testid="fire-string" onClick={() => onChange({ target: { value: 'OPEN,CLOSED' } })}>
      fire
    </button>
  ),
}));
...
describe('EnumControl string value handling', () => {
  it('splits a comma-joined string value into an array', () => {
    const onChange = vi.fn();
    render(<FilterControl<Row> column={column} label="Status" draft={emptyDraft(column)} onChange={onChange} />);
    fireEvent.click(screen.getByTestId('fire-string'));
    expect(onChange).toHaveBeenCalledWith({ selected: ['OPEN', 'CLOSED'] });
  });
});
