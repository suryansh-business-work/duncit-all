/**
 * How many GraphQL requests this surface has out right now.
 *
 ...
 * transport for free.
 *
 * Lives beside `TopProgressBar` so every surface that draws the bar — the 17
 * portals through `@duncit/shell`, the pet store through its own client — reads
 * the same counter rather than keeping one each (rule 40).
 */
let inFlight = 0;
