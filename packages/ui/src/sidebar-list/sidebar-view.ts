...
export function sortOptionsFor(t: Translate, items: SidebarItem[]): SidebarOption<SidebarSort>[] {
  const options: SidebarOption<SidebarSort>[] = [
    { value: 'list', label: t('ui.sidebarList.listOrder') },
    { value: 'name-asc', label: t('ui.sidebarList.nameAZ') },
    { value: 'name-desc', label: t('ui.sidebarList.nameZA') },
  ];
  if (items.some((item) => item.count !== undefined)) {
    options.push({ value: 'used', label: t('ui.sidebarList.mostUsed') });
  }
  if (items.some((item) => item.updatedAt)) {
    options.push({ value: 'recent', label: t('ui.sidebarList.recentlyUpdated') });
  }
  return options;
}
 ...
 *
 * The email lists count something — sends for a template, templates for a
 * fragment — and must show the zeroes: a row nothing has ever used is
...
export function statusOptionsFor(t: Translate): SidebarOption<SidebarStatus>[] {
  return [
    { value: 'all', label: t('ui.sidebarList.anyStatus') },
    { value: 'active', label: t('ui.sidebarList.activeOnly') },
    { value: 'off', label: t('ui.sidebarList.switchedOffOnly') },
  ];
}
...
export function emptyMessage(
  ...
  if (needle) return t('ui.sidebarList.nothingMatchesSearch', { vars: { needle } });
  if (input.filtered) return t('ui.sidebarList.nothingMatchesTheFilters');
...
export function countLabel(t: Translate, shown: number, total: number): string {
  if (shown === total) return t('ui.sidebarList.totalCount', { vars: { count: total } });
  return t('ui.sidebarList.showingOfTotal', { vars: { shown, total } });
}
