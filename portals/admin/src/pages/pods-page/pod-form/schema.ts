...
/**
 * Zod port of the former yup `podFormSchema`. Mirrors every required/min/max,
 * conditional (`.when` -> `superRefine`) and validation message 1:1 so the
 * admin Pod form behaves identically after the RHF migration.
 */
export const podFormSchema = z
  .object({
    ...
    meeting_notes: z
      ...
      .default(''),
    pod_hosts_id: z.array(z.string()).min(1, 'Add at least one host'),
    pod_description: z.string().trim().min(10, 'Add a longer description').min(1, 'Description required'),
    pod_date_time: z.string().min(1, 'Start date/time required'),
    pod_end_date_time: z.string().default(''),
  ...
  .superRefine((values, ctx) => {
    ...
    if (values.pod_date_time && new Date(values.pod_date_time).getTime() <= Date.now()) {
    ...
    if (
      ...
      new Date(values.pod_end_date_time).getTime() <= new Date(values.pod_date_time).getTime()
    ) {
...
export type PodFormSchema = z.infer<typeof podFormSchema>;
