...
import { makeAdDetail } from '../mocks';
import { renderWithProviders } from '../testkit';

describe('AdMediaCard', () => {
  it('renders an image creative for an IMAGE ad', () => {
    const { container } = renderWithProviders(<AdMediaCard ad={makeAdDetail({ ad_type: 'IMAGE' })} />);
  ...
  it('renders a video creative for a VIDEO ad', () => {
    const { container } = renderWithProviders(
      <AdMediaCard ad={makeAdDetail({ ad_type: 'VIDEO', media_url: 'https://cdn/clip.mp4' })} />,
    );
