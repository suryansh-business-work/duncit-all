...
// adapter computes and lets us fire its onLogout / onDenied callbacks.
  ...
  return {
    ...
    AppShell: (props: Record<string, any>) => (
      <div data-testid="shell">
        ...
        <span data-testid="profile-to">{props.profileTo}</span>
        <button onClick={props.onLogout}>logout</button>
        <button onClick={props.onDenied}>denied</button>
        {props.children}
      </div>
    ),
  };
...
describe('AppShell adapter', () => {
  it('grants access and passes the user through when a manager is signed in', () => {
    userMock.value = { user: { roles: ['ADS_MANAGER'] }, loading: false, logout: vi.fn() };
    renderShell();
    ...
    expect(screen.getByTestId('profile-to')).toHaveTextContent('/profile');
  ...
  it('denies access by clearing the token via onDenied', () => {
    userMock.value = { user: { roles: ['OTHER'] }, loading: false, logout: vi.fn() };
    setToken('tok');
    renderShell();
  ...
  it('logs out: clears the token, calls context logout and routes to /login', () => {
    const logout = vi.fn();
    userMock.value = { user: { roles: ['ADS_MANAGER'] }, loading: false, logout };
    setToken('tok');
    renderShell();
