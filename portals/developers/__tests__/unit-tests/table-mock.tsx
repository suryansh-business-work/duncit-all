...
/**
 * A lightweight stand-in for `@duncit/table` used by the developers-portal table
 ...
 * real), and wires `onRowClick`/`getRowId`/`emptyText`/`refetchRef`. Rows come
 * from `fetchRows`, which the page builds from the (mocked) `useApolloTableFetch`.
 *
...
/** Mirror of the production `formatDateCell` used by the table pages. */
export function formatDateCell(iso: string | null, _fmt: string): string {
  return iso ? `fmt:${iso}` : '—';
}
