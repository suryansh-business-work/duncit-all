...
describe('App routing', () => {
  ...
  it('renders the shared profile page inside the shell behind auth', () => {
    setToken('tok');
    renderWithProviders(<App />, { initialEntries: ['/profile'] });
    expect(screen.getByTestId('shell')).toContainElement(screen.getByText('PROFILE PAGE'));
  });
  ...
  it('redirects unknown routes to the dashboard', () => {
    setToken('tok');
    renderWithProviders(<App />, { initialEntries: ['/nowhere'] });
    expect(screen.getByText('DASHBOARD PAGE')).toBeInTheDocument();
  });
});
