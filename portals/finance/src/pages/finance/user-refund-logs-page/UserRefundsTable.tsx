...
export default function UserRefundsTable({ fetchRows, refetchRef, onOpen }: Readonly<Props>) {
  ...
  const columns = useMemo<DuncitColumn<UserRefundRow>[]>(() => {
    ...
    return [
      ...
      {
        // Falls back to the payment total when no figure was written, so there
        // is no single stored path the server can order or match on.
        field: 'refund_amount',
      ...
      {
        // Derived from status (SUCCESS = part refund): sorts on status, and the
        // Status filter already answers it — there is no stored flag to match.
        field: 'partial',
