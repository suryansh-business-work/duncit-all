...
describe('Hosts console app config', () => {
  it('declares its identity, storage keys and one dashboard entry', async () => {
    const appConfig = await loadConfig();
    expect(appConfig.key).toBe('hosts');
    expect(appConfig.fullName).toBe('Duncit Hosts');
    expect(appConfig.tokenKey).toBe('hosts_token');
    expect(appConfig.colorModeKey).toBe('hosts_color_mode');
  ...
  it('gates on its default roles and ships its own login image by default', async () => {
    ...
    const appConfig = await loadConfig();
    expect(appConfig.requiredRoles).toEqual(['ALL_HOSTS_ACCESS', 'ONBOARDING_MANAGER']);
    expect(appConfig.loginImage).toBe('https://images.pexels.com/photos/1181395/pexels-photo-1181395.jpeg');
  });

  it('honours the environment overrides for roles and the login image', async () => {
    vi.stubEnv('VITE_REQUIRED_ROLES', 'ALL_HOSTS_ACCESS, SUPER_ADMIN');
    vi.stubEnv('VITE_LOGIN_IMAGE', 'https://cdn.duncit.com/login/hosts.jpg');
    const appConfig = await loadConfig();
    expect(appConfig.requiredRoles).toEqual(['ALL_HOSTS_ACCESS', 'SUPER_ADMIN']);
    expect(appConfig.loginImage).toBe('https://cdn.duncit.com/login/hosts.jpg');
  });
});
