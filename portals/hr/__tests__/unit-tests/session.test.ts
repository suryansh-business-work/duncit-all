...
describe('hasAppAccess', () => {
  it('grants access to the configured HR_MANAGER role', () => {
    expect(hasAppAccess(['HR_MANAGER'])).toBe(true);
  });
  ...
  it('uses HR_MANAGER as the default required role', () => {
    expect(appConfig.requiredRoles).toContain('HR_MANAGER');
  });
});
