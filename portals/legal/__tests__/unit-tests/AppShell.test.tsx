...
describe('AppShell', () => {
  ...
  it('renders the shell and page for an authorised user', () => {
    ctx.state = {
      user: { full_name: 'Jane Doe', roles: ['LEGAL_MANAGER'], profile_photo: '' },
      loading: false,
      logout: vi.fn(),
    };
  ...
  it('derives initials from first/last, email, then the app name', () => {
    ctx.state = {
      user: { first_name: 'Ann', last_name: 'Bell', roles: ['LEGAL_MANAGER'] },
      loading: false,
      logout: vi.fn(),
    };
    ...
    ctx.state = { user: { email: 'zoe@duncit.com', roles: ['LEGAL_MANAGER'] }, loading: false, logout: vi.fn() };
    const b = renderWithProviders(<AppShell>{child}</AppShell>);
    expect(screen.getByText('Z')).toBeInTheDocument();
    b.unmount();

    ctx.state = { user: { roles: ['LEGAL_MANAGER'] }, loading: false, logout: vi.fn() };
    renderWithProviders(<AppShell>{child}</AppShell>);
  ...
  it('logs out: clears the token and calls the context logout', () => {
    const logout = vi.fn();
    ctx.state = { user: { full_name: 'Jane Doe', roles: ['LEGAL_MANAGER'] }, loading: false, logout };
    setToken('tok');
    renderWithProviders(<AppShell>{child}</AppShell>);
  ...
  it('toggles color mode and opens then closes the mobile navigation drawer', async () => {
    ctx.state = { user: { full_name: 'Jane Doe', roles: ['LEGAL_MANAGER'] }, loading: false, logout: vi.fn() };
    const { container } = renderWithProviders(<AppShell>{child}</AppShell>);
