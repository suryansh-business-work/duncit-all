...
describe('PoliciesPage', () => {
  ...
  it('toggles active state and edits sort order in the dialog', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Edit' }));
    const dialog = await screen.findByRole('dialog');
    // Switching visibility off relabels the switch from "Active" to "Hidden".
    ...
    expect(within(dialog).getByLabelText(/Slug/)).toHaveValue('privacy');
  ...
  it('edits a policy that has no stored content and closes via the backdrop', async () => {
    ...
    expect(within(dialog).getByTestId('rich-text-editor')).toHaveTextContent('');
