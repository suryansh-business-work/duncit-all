...
import { translatedFor, type LocaleRow, type TranslationGroupRow } from '../../lib/queries';
...
function Completeness({ translated, total }: Readonly<CompletenessProps>) {
  return (
    <Typography variant="body2" color={completenessColor(translated, total)} sx={{ fontWeight: 600 }}>
      {translated} / {total}
    </Typography>
  );
}
...
export function getTranslationGroupColumns(
  ...
  return [
    ...
    {
      field: 'key_count',
      headerName: t('localization.translations.colKeys'),
      type: 'number',
    },
    ...localeColumns,
  ];
