describe('Duncit Partners login', () => {
  ...
  it('shows the Duncit Partners sign-in form', () => {
    cy.visit('/login');
  ...
  it('surfaces a server-side error on invalid credentials', () => {
    cy.mockGraphql({
      PartnerLogin: { errors: [{ message: 'Invalid email or password', extensions: { code: 'UNAUTHENTICATED' } }] },
    });
    cy.visit('/login');
