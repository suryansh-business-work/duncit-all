 ...
 * Partners is portal-gate-exempt, so `requiredRoles` is empty and sign-in is not
 * gated on the client (each partner area is gated by SectionGate instead) — the
 * helpers are still used for token storage.
 */
