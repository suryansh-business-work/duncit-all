...
export default function HostStepContent({ step, s1, s2, s3, set1, set2, set3, openPicker }: Readonly<Props>) {
  ...
  if (step === 0) {
    return (
      <Stack spacing={2}>
        <TextField label={t('partners.becomeHostPage.fullName')} required autoComplete="name" value={s1.full_name} onChange={(e) => set1({ ...s1, full_name: e.target.value })} />
        <TextField label={t('shell.common.email')} type="email" required autoComplete="email" value={s1.email} disabled helperText={t('partners.becomeHostPage.lockedToYourDuncitAccount')} slotProps={{
          input: { readOnly: true }
        }} />
        <TextField label={t('shell.common.phone')} required type="tel" autoComplete="tel" value={s1.phone} onChange={(e) => set1({ ...s1, phone: e.target.value })} />
  ...
  if (step === 1) {
    return (
      <Stack spacing={2}>
        ...
        {s2.passport_photo_url && <Box component="img" src={s2.passport_photo_url} alt={t('partners.becomeHostPage.passportSizePhoto')} sx={{ width: 120, height: 120, objectFit: 'cover', borderRadius: 1, border: 1, borderColor: 'divider' }} />}
  ...
  if (step === 2) {
    return (
      <Stack spacing={2}>
        ...
        <TextField label={t('partners.becomeHostPage.fullAddress')} required multiline minRows={3} autoComplete="street-address" value={s3.full_address} onChange={(e) => set3({ ...s3, full_address: e.target.value })} />
      </Stack>
    );
  }
  return (
