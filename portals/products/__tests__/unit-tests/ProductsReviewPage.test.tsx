...
import ProductsReviewPage from '../../src/pages/ecomm/ProductsReviewPage';
...
describe('ProductsReviewPage', () => {
  it('renders the header, status tabs and opens the review dialog', async () => {
    ...
    renderWithProviders(<ProductsReviewPage />);
    expect(screen.getByText('Products Reviews')).toBeInTheDocument();
  ...
  it('shows a success message and reloads after a review', async () => {
    __setTableRows([makeProductListingRow({ created_at: null })]);
    renderWithProviders(<ProductsReviewPage />, { mocks: [reviewProductListingMock({ status: 'APPROVED' })] });
    await waitFor(() => expect(screen.getByRole('button', { name: 'Review' })).toBeInTheDocument());
    fireEvent.click(screen.getByRole('button', { name: 'Review' }));
