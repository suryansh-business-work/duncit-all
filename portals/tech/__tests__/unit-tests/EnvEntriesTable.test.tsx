...
import EnvEntriesTable from '../../src/pages/environment/EnvEntriesTable';
import type { EnvEntry } from '../../src/pages/environment/queries';
import { makeEnvEntry } from '../mocks/env-entry.mock';

const fetchFor = (entries: EnvEntry[]) => vi.fn(async () => ({ rows: entries, total: entries.length }));
...
describe('EnvEntriesTable', () => {
  ...
  it('renders default/active/tested/assigned variants and wires actions', async () => {
    const onEdit = vi.fn(),
      ...
      onTest = vi.fn();
    const entries = [
      makeEnvEntry({ id: 'a', name: 'Default A', is_default: true, is_active: true, last_test_ok: true, last_tested_at: '2026-01-01T00:00:00Z', assigned_portals: ['crm', 'ads'] }),
      makeEnvEntry({ id: 'b', name: 'Off B', is_default: false, is_active: false, last_test_ok: false, last_tested_at: '2026-01-01T00:00:00Z' }),
      makeEnvEntry({ id: 'c', name: 'Untested C' }),
      makeEnvEntry({ id: 'd', name: 'Dateless D', last_test_ok: true, last_tested_at: null }),
    ];
    render(
      <EnvEntriesTable
