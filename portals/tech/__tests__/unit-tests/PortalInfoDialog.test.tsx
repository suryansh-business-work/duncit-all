...
import PortalInfoDialog from '../../src/pages/environment/portal-mapping/PortalInfoDialog';
...
describe('PortalInfoDialog', () => {
  ...
  it('groups entries by category and shows default/active/off + descriptions', () => {
    const row: PortalRow = makePortalRow({
      portal: makePortalListItem({ key: 'crm', name: 'CRM', kind: 'PORTAL' }),
      entries: [
        makeEnvEntry({ id: '1', name: 'Mail A', category: 'EMAIL', description: 'primary', is_default: true, is_active: true }),
        makeEnvEntry({ id: '2', name: 'Twilio B', category: 'TWILIO', description: '', is_active: false }),
      ],
    });
