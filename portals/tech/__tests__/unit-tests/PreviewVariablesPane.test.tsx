...
import PreviewVariablesPane from '../../src/pages/email-templates-page/PreviewVariablesPane';
...
describe('PreviewVariablesPane — variables tab', () => {
  ...
  it('shows the empty-declared hint when there are no declared variables', () => {
    renderPane({ tab: 'code', draft: makeTpl({ variables: [] }) });
  ...
  it('edits a declared variable key + description and deletes a row', () => {
    const draft = makeTpl({ variables: [{ key: 'name', description: 'the user name' }] });
    const setDraft = vi.fn();
    render(
      <PreviewVariablesPane
        draft={draft}
  ...
  it('falls back to an empty description when none is set', () => {
    const draft = makeTpl({ variables: [{ key: 'name' }] });
    render(
      <PreviewVariablesPane
        draft={draft}
        setDraft={vi.fn()}
