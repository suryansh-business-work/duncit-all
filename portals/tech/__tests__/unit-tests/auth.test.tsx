...
import { makeJwtSettings } from '../mocks/auth.mock';
...
import JwtExpirySection from '../../src/pages/JwtExpirySection';
...
describe('JwtExpirySection', () => {
  ...
  it('parses a "45m" expiry into minutes and updates the summary on change', () => {
    m.query = { data: { appSettings: makeJwtSettings({ jwt_expires_in: '45m', updated_at: '2026-01-01T00:00:00.000Z' }) }, loading: false, error: undefined, refetch: vi.fn() };
    render(<JwtExpirySection onToast={vi.fn()} />);
  ...
  it('parses a numeric-seconds expiry into days', () => {
    m.query = { data: { appSettings: makeJwtSettings({ jwt_expires_in: '172800' }) }, loading: false, error: undefined, refetch: vi.fn() };
    render(<JwtExpirySection onToast={vi.fn()} />);
    expect(screen.getByText(/expire after 2d/)).toBeInTheDocument();
  });

  it('falls back to 7 days for an unparseable expiry', () => {
    m.query = { data: { appSettings: makeJwtSettings({ jwt_expires_in: 'weird' }) }, loading: false, error: undefined, refetch: vi.fn() };
    render(<JwtExpirySection onToast={vi.fn()} />);
    expect(screen.getByText(/expire after 7d/)).toBeInTheDocument();
  });

  it('defaults to 7 days when the expiry is null', () => {
    m.query = { data: { appSettings: makeJwtSettings({ jwt_expires_in: null }) }, loading: false, error: undefined, refetch: vi.fn() };
    render(<JwtExpirySection onToast={vi.fn()} />);
    expect(screen.getByText(/expire after 7d/)).toBeInTheDocument();
  });
