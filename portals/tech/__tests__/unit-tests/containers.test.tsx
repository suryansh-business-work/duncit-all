...
const { notify, confirmMock, refetchSpy } = m;
const st = m;

// The sample entry is a typed factory value referenced only inside the deferred
// stub bodies below, so a module-scope const is safe under vi.mock hoisting.
const sample = makeEnvEntry({ id: 'ent1', name: 'SMTP One', category: 'EMAIL', assigned_portals: ['crm'], config: [{ key: 'host', value: 'smtp' }] });
...
vi.mock('../../src/pages/environment/EnvEntriesTable', () => ({
  default: (p: {
    refetchRef: { current: (() => void) | null };
    toolbarActions?: React.ReactNode;
    onEdit: (e: typeof sample) => void;
    onDelete: (e: typeof sample) => void;
    onSetDefault: (e: typeof sample) => void;
    onTest: (e: typeof sample) => void;
  }) => {
    if (m.assignRefetch) p.refetchRef.current = m.refetchSpy;
    return (
      <div data-testid="env-table">
        {p.toolbarActions}
        <button type="button" onClick={() => p.onEdit(sample)}>tbl-edit</button>
        <button type="button" onClick={() => p.onDelete(sample)}>tbl-delete</button>
        <button type="button" onClick={() => p.onSetDefault(sample)}>tbl-default</button>
        <button type="button" onClick={() => p.onTest(sample)}>tbl-test</button>
      </div>
    );
  },
}));

vi.mock('../../src/pages/environment/env-entry', async (io) => {
  const actual = await io<typeof import('../../src/pages/environment/env-entry')>();
  return {
    ...actual,
    EnvEntryForm: (p: {
      open: boolean;
...
vi.mock('../../src/pages/environment/test-panels', () => ({
  default: (p: { entry: typeof sample | null; onClose: () => void }) =>
    p.entry ? (
...
vi.mock('../../src/pages/environment/portal-mapping/PortalMappingTable', () => ({
  default: (p: {
...
vi.mock('../../src/pages/environment/portal-mapping/PortalInfoDialog', () => ({
  default: (p: { row: unknown; onClose: () => void }) =>
    p.row ? <button type="button" onClick={p.onClose}>info-close</button> : null,
}));
vi.mock('../../src/pages/environment/PortalEnvDrawer', () => ({
...
import EnvironmentPage from '../../src/pages/environment/index';
import EnvVariablesTab from '../../src/pages/environment/EnvVariablesTab';
