...
import { errorRateColor } from '../labels';
...
export default function FieldsTable({ fields }: Readonly<Props>) {
  ...
  const columns = useMemo<DuncitColumn<FieldUsage>[]>(
    () => [
      ...
      { field: 'kind', headerName: t('tech.graphqlMonitor.colType'), width: 130, cellRenderer: renderKind, valueGetter: (row) => row.kind },
      { field: 'return_type', headerName: t('tech.graphqlMonitor.colReturns'), width: 200, valueGetter: (row) => row.return_type },
      { field: 'referenced', headerName: t('tech.graphqlMonitor.colReferenced'), width: 120, valueGetter: (row) => formatCount(row.referenced) },
      { field: 'avg_ms', headerName: t('tech.graphqlMonitor.colResolverAvg'), width: 120, valueGetter: (row) => formatMs(row.avg_ms) },
      { field: 'max_ms', headerName: t('tech.graphqlMonitor.colResolverMax'), width: 120, valueGetter: (row) => formatMs(row.max_ms) },
      ...
      { field: 'sampled_calls', headerName: t('tech.graphqlMonitor.colSampledCalls'), width: 120, hide: true, valueGetter: (row) => formatCount(row.sampled_calls) },
