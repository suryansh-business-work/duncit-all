...
export default function EndpointsTable({ endpoints }: Readonly<Props>) {
  ...
  const columns = useMemo<DuncitColumn<StressEndpoint>[]>(
    () => [
      { field: 'key', headerName: t('tech.stress.colEndpoint'), flex: 1, minWidth: 220, valueGetter: (row) => row.key },
      { field: 'requests', headerName: t('tech.stress.colRequests'), width: 110, valueGetter: (row) => formatCount(row.requests) },
      { field: 'errors', headerName: t('tech.stress.colErrors'), width: 100, valueGetter: (row) => formatPct(errorRate(row)) },
      { field: 'avg_ms', headerName: t('tech.stress.colAvg'), width: 100, valueGetter: (row) => formatMs(row.avg_ms) },
      { field: 'p50_ms', headerName: t('tech.stress.colP50'), width: 100, valueGetter: (row) => formatMs(row.p50_ms) },
      { field: 'p95_ms', headerName: t('tech.stress.colP95'), width: 100, valueGetter: (row) => formatMs(row.p95_ms) },
      { field: 'p99_ms', headerName: t('tech.stress.colP99'), width: 100, valueGetter: (row) => formatMs(row.p99_ms) },
    ],
    [t]
  );
