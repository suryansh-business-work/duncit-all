...
import { formatMjml } from './mjmlFormat';

describe('formatMjml', () => {
  it('re-indents MJML by nesting depth (splitting on whitespace between tags)', () => {
    const out = formatMjml('<mjml> <mj-body> <mj-section> <mj-column> </mj-column> </mj-section> </mj-body> </mjml>');
    expect(out).toBe(
      [
        '<mjml>',
        '  <mj-body>',
  ...
  it('treats self-closing tags as leaves (no extra indent) and drops blank lines', () => {
    ...
    expect(out).toBe(['<mjml>', '  <mj-body>', '    <mj-divider />', '  </mj-body>', '</mjml>'].join('\n'));
  });
