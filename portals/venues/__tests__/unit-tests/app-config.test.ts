...
describe('Venues console app config', () => {
  it('declares its identity, storage keys and one dashboard entry', async () => {
    const appConfig = await loadConfig();
    expect(appConfig.key).toBe('venues');
    expect(appConfig.fullName).toBe('Duncit Venues');
    expect(appConfig.tokenKey).toBe('venues_token');
    expect(appConfig.colorModeKey).toBe('venues_color_mode');
  ...
  it('gates on its default roles and ships its own login image by default', async () => {
    ...
    const appConfig = await loadConfig();
    expect(appConfig.requiredRoles).toEqual(['ALL_VENUES_ACCESS', 'ONBOARDING_MANAGER']);
    expect(appConfig.loginImage).toBe('https://images.pexels.com/photos/260922/pexels-photo-260922.jpeg');
  });

  it('honours the environment overrides for roles and the login image', async () => {
    vi.stubEnv('VITE_REQUIRED_ROLES', 'ALL_VENUES_ACCESS, SUPER_ADMIN');
    vi.stubEnv('VITE_LOGIN_IMAGE', 'https://cdn.duncit.com/login/venues.jpg');
    const appConfig = await loadConfig();
    expect(appConfig.requiredRoles).toEqual(['ALL_VENUES_ACCESS', 'SUPER_ADMIN']);
    expect(appConfig.loginImage).toBe('https://cdn.duncit.com/login/venues.jpg');
  });
});
