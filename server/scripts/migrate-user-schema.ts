...
async function migrate() {
  ...
  const userRelationships = db.collection('userrelationships');
  const podFollowers = db.collection('podfollowers');
  const clubFollowers = db.collection('clubfollowers');
  const userSavedPods = db.collection('usersavedpods');
  ...
  while (await cursor.hasNext()) {
    ...
    try {
      ...
      const authDoc: Record<string, any> = prune({
      ...
      await Promise.all([
        safeInsert(userRoles, rolesRows),
        safeInsert(userRelationships, followRows),
        safeInsert(podFollowers, podFollowerRows),
        safeInsert(clubFollowers, clubFollowerRows),
        safeInsert(userSavedPods, savedPodRows),
        safeInsert(userInterests, interestRows),
      ]);
  ...
  if (!dryRun) {
    ...
    const dirty = users.find({ 'counters.followers_count_dirty': true }).project({ _id: 1 });
    while (await dirty.hasNext()) {
      const r: any = await dirty.next();
      const count = await userRelationships.countDocuments({ following_id: r._id });
