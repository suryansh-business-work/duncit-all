...
import { argumentsFor, selectionFor } from './gql-arguments';
...
/** One GraphQL request. Transport failures surface; per-field errors do not. */
...
async function sweep(kind: 'query' | 'mutation', token: string | undefined, fields: string[]) {
  ...
  sent += 1;
  const result = await post(token, `${kind} {\n${fields.join('\n')}\n}`);
...
afterAll(async () => {
  ...
  console.log(
    `resolver sweep: ${queryFields.length} queries + ${mutationFields.length} mutations, ` +
      `${sent} request(s), ${unswept} field-pass(es) left unswept by the budget, ` +
...
describe('every Query', () => {
  it('answers an admin without the server falling over', async () => {
    await runAll('query', adminToken(), queryFields);

    await expect(post(adminToken(), '{ _ping }')).resolves.toMatchObject({ data: { _ping: 'pong' } });
  });
...
describe('every Mutation', () => {
  it('answers an admin without the server falling over', async () => {
    await runAll('mutation', adminToken(), mutationFields);

    await expect(post(adminToken(), 'mutation { _noop }')).resolves.toMatchObject({
      data: { _noop: true },
    });
  });
