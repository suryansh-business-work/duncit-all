...
/**
 * The shape of an In App Prompt — every prompt the product sends to a model.
 *
 * Two roles ship: the SYSTEM turn (the standing instruction, the same on every
 * call) and the USER turn (the per-call payload, whose fixed wording used to be
 * a template literal buried at the call site). Both are operator-owned now,
 * because a reviewer who can edit "you are the content-safety reviewer" but not
 * "Review this pod:" only half-controls what the model is asked.
 *
 * Everything here except `content` is CODE-owned: it describes where the prompt
 * is wired in, which the operator cannot change from a portal. `content` is the
 * one field the library takes over after the first seed.
 */
