 ...
 * id — so holding the role grants a manager their patch and nobody else's.
...
export const regionResolvers = {
  Query: {
    ...
    regionClubAdminCandidates: async (
      ...
      const user = requireRole(ctx, REGIONAL_RW);
      const limit = Math.min(MAX_CANDIDATE_LIMIT, Math.max(1, args.limit ?? DEFAULT_CANDIDATE_LIMIT));
    ...
    regionHostPods: async (
      ...
      const user = requireRole(ctx, REGIONAL_RW);
      return regionService.hostPods(user.id, args.host_user_id, args.query);
    },
  },
  Mutation: {
    renameMyRegion: async (
      _p: unknown,
