...
describe('withdrawals', () => {
  ...
  /*
    Two floors sit in front of the balance check, both from Finance >
    Withdrawal Settings: a Host cannot withdraw until the WALLET reaches the
    minimum, and cannot withdraw an AMOUNT below it. Every case here funds
    above that floor so the rule being tested is the one that fails.
  */
  it('blocks a withdrawal under the role minimum, on the balance and on the amount', async () => {
    const poor = await fund(500);
    await expect(
      walletService.requestWithdrawal(String(poor._id), { amount: 400, payout_method: 'UPI', upi_id: 'a@upi' })
    ).rejects.toThrow(/balance reaches/i);

    const host = await fund(1500);
    await expect(
      walletService.requestWithdrawal(String(host._id), { amount: 400, payout_method: 'UPI', upi_id: 'a@upi' })
  ...
  it('blocks overdraw and missing account details', async () => {
    const host = await fund(1500);
    await expect(
      walletService.requestWithdrawal(String(host._id), { amount: 1900, payout_method: 'UPI', upi_id: 'a@upi' })
    ).rejects.toThrow(/insufficient/i);
    await expect(
      walletService.requestWithdrawal(String(host._id), { amount: 1100, payout_method: 'UPI' })
    ).rejects.toThrow(/account details/i);
  });

  it('rejecting a withdrawal refunds the wallet; paying it does not', async () => {
    const host = await fund(5000);
    const reject = await walletService.requestWithdrawal(String(host._id), { amount: 1400, payout_method: 'IMPS', account_number: '123', ifsc_code: 'HDFC0001' });
    expect((await walletService.getMyWallet(String(host._id))).balance).toBe(3600);

    await walletService.reviewWithdrawal(reject.id, 'REJECTED', 'Bad account', new Types.ObjectId().toString());
    expect((await walletService.getMyWallet(String(host._id))).balance).toBe(5000);

    const pay = await walletService.requestWithdrawal(String(host._id), { amount: 1300, payout_method: 'UPI', upi_id: 'a@upi' });
    ...
    expect((await walletService.getMyWallet(String(host._id))).balance).toBe(3700);
  });

  it('serves the withdrawalRequestsTable page with search, filter, sort and paging', async () => {
    const a = await fund(4000);
    const b = await fund(4000);
    const c = await fund(4000);
    // Every amount clears the ₹1000 role minimum — the ordering being tested
    // is the table's, not the withdrawal rule's.
    await walletService.requestWithdrawal(String(a._id), { amount: 3000, payout_method: 'UPI', upi_id: 'alpha@upi' });
    await walletService.requestWithdrawal(String(b._id), { amount: 2000, payout_method: 'UPI', upi_id: 'beta@upi' });
    const paid = await walletService.requestWithdrawal(String(c._id), { amount: 1000, payout_method: 'UPI', upi_id: 'gamma@upi' });
    ...
    expect(asc.rows.map((w) => w.amount)).toEqual([1000, 2000, 3000]);
    const page2 = await walletService.withdrawalsTable({ sort_by: 'amount', sort_dir: 'asc', page: 2, page_size: 1 });
    expect(page2.rows.map((w) => w.amount)).toEqual([2000]);
  ...
  it('validates the review (reason required, only pending)', async () => {
    const host = await fund(3000);
    const w = await walletService.requestWithdrawal(String(host._id), { amount: 1200, payout_method: 'UPI', upi_id: 'a@upi' });
