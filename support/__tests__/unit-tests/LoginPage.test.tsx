...
const submitLogin = () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: /open support console/i }));
};
...
describe('LoginPage', () => {
  it('renders the sign-in form', async () => {
    renderLogin([]);
    expect(await screen.findByText(/sign in to duncit support/i)).toBeInTheDocument();
  });
  ...
  it('logs in and navigates to the default route', async () => {
    renderLogin([loginResult('tok', ['SUPPORT_MANAGER'])]);
    await screen.findByText(/sign in to duncit support/i);
    submitLogin();
  ...
  it('honours a safe redirect query param', async () => {
    renderLogin([loginResult('tok', ['SUPPORT_MANAGER'])], ['/login?redirect=%2Ftickets']);
    await screen.findByText(/sign in to duncit support/i);
    submitLogin();
  ...
  it('falls back to the originating location from router state', async () => {
    renderLogin(
      [loginResult('tok', ['SUPPORT_MANAGER'])],
  ...
  it('logs in with Google', async () => {
    ...
    renderLogin([
      { request: { query: LOGIN_GOOGLE }, variableMatcher: () => true, result: { data: { loginWithGoogle: { token: 'tok', user: { user_id: 'u1', first_name: 'A', last_name: 'B', email: 'a@b.com', roles: ['SUPPORT_MANAGER'] } } } } },
    ]);
    await screen.findByText(/sign in to duncit support/i);
