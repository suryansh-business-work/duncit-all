...
const ImageOverlay = styled(Box)(({ theme }) => {
  ...
  return {
    ...
    alignItems: 'center',
    background: isDark
      ? `linear-gradient(180deg, ${alpha('#000', 0.55)} 0%, ${alpha('#000', 0.78)} 100%)`
      : `linear-gradient(180deg, ${alpha(theme.palette.primary.main, 0.45)} 0%, ${alpha('#000', 0.62)} 100%)`,
    color: '#fff',
  };
});
...
export default function AuthSplitLayout({ title, subtitle, children }: Props) {
  ...
  return (
    <Shell>
      ...
      <ImagePane image={appConfig.loginImage}>
        <ImageOverlay>
          ...
          <Box sx={{ mt: 'auto', textAlign: 'center', maxWidth: 520, mx: 'auto' }}>
            <Typography variant="h5" sx={{ fontWeight: 700, mb: 1, textShadow: '0 2px 12px rgba(0,0,0,0.45)' }}>
              {appConfig.fullName}
            </Typography>
            <Typography variant="body2" sx={{ opacity: 0.92, textShadow: '0 2px 10px rgba(0,0,0,0.45)' }}>
              {appConfig.tagline}
            </Typography>
          </Box>
        </ImageOverlay>
