...
describe('fetchNavGroups', () => {
  ...
  it('queries the given site and groups the links for the area', async () => {
    ...
    const groups = await fetchNavGroups('MAIN', 'HEADER', fallback);
  ...
  it('falls back when no links match the requested area', async () => {
    ...
    await expect(fetchNavGroups('PARTNERS', 'FOOTER', fallback)).resolves.toBe(fallback);
  });

  it('falls back when the request fails', async () => {
    fetchMock.mockResolvedValue({ ok: false, json: async () => ({}) });
    await expect(fetchNavGroups('ADS', 'HEADER', fallback)).resolves.toBe(fallback);
  });
});
